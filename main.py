...
def getInput(prompt: str, verifier) -> int:
    while True:
        try:
...
def main():
    ...
    file_choice = dict_paths[getInput(">", lambda x: x in dict_paths.keys())]
    ext = file_choice.suffix

    p.print_info(f"Selected file: {file_choice}")
    print(f"1. Conversion\n2. Remove Audio\n3. Extract Audio\n4. Trim\n5. Exit")

    process_choice = getInput(">", lambda x: x in {1, 2, 3, 4, 5})
    if process_choice == 5:
        return
    ...
    if process_choice == 1:
        ...
        conversion_choice = getInput(">", lambda x: dict_conversions.keys().__contains__(x))
        ...
        start_time = getInput("Start time (in seconds): ", lambda x: {
            duration > x >= 0
        })
        end_time = getInput("End time (in seconds): ", lambda x: {
